...
export default function ProductDetailPage() {
  ...
  if (loading) return <><SiteHeader /><main className="content page-shell"><p className="empty">商品情報を読み込んでいます…</p></main></>;
  if (error || !product) return <><SiteHeader /><main className="content page-shell"><Link className="breadcrumb" href="/products">← 商品一覧へ戻る</Link><div className="notice">{error || "商品が見つかりません。"}</div></main></>;
  return (
    <>
      <SiteHeader />
      <main className="content page-shell">
        <Link className="breadcrumb" href="/products">← 商品一覧へ戻る</Link>
        <p className="eyebrow">PRODUCT DETAIL</p>
        <section className="product-summary">
          <h1>{product.name}</h1>
          <p>{[product.maker, product.quantity].filter(Boolean).join(" ・ ") || ""}</p>
          {product.jan && <small>JAN {product.jan}</small>}
        </section>
        <div className="section-heading review-heading">
          <div><p className="eyebrow">LATEST REVIEWS</p><h2>見つかった店舗</h2></div>
          <Link className="row-link" href={`/submit?product=${product.id}`}>この商品を投稿 →</Link>
        </div>
        {reviews.length === 0 ? <p className="empty">まだ口コミがありません。</p> : (
          <div className="review-list">
            {reviews.map((review) => (
              <article className="review-card" key={review.id}>
                <div className="review-card-top">
                  <h3>{review.stores?.[0]?.name ?? "店舗不明"}</h3>
                  <span>{review.stores?.[0]?.area}</span>
                </div>
                <p className="review-meta">
                  <span>確認日 {new Date(review.observed_at).toLocaleString("ja-JP")}</span>
                  <span className={`stock-badge stock-${review.stock_status}`}>{stockLabels[review.stock_status] ?? "不明"}</span>
                  {review.price !== null && <span className="review-price">{review.price.toLocaleString("ja-JP")}円</span>}
                </p>
